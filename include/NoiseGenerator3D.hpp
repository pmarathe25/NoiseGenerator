...
namespace StealthNoiseGenerator {
    namespace {
        ...
        constexpr void fillCube(int internalX, int internalY, int internalZ, int fillStartX, int fillStartY, int fillStartZ,
            ...
            const TileMapF<scaleZ>& attenuationsZ, float multiplier = 1.0f) {
                    ...
                    for (int i = 0; i < maxValidX; ++i) {
                        ...
                        if constexpr (overwrite::value) {
                            generatedNoiseMap[index++] = interpolate3D(topLeft0, topRight0, bottomLeft0, bottomRight0,
                                ...
                                attenuationX, attenuationY, attenuationZ);
                        } else {
                            generatedNoiseMap[index++] += interpolate3D(topLeft0, topRight0, bottomLeft0, bottomRight0,
                                topLeft1, topRight1, bottomLeft1, bottomRight1,
                                attenuationX, attenuationY, attenuationZ) * multiplier;
                        }
                    }
    ...
    constexpr GeneratedNoiseType& generate(GeneratedNoiseType& generatedNoiseMap, Seed&& seed = getCurrentTime(),
        ...
        if constexpr (height == 1) {
            return generate<width, length, scaleX, scaleY, overwrite>(generatedNoiseMap, std::forward<Seed&&>(seed),
                std::forward<Distribution&&>(distribution), multiplier);
        } else if constexpr (length == 1) {
            return generate<width, height, scaleX, scaleZ, overwrite>(generatedNoiseMap, std::forward<Seed&&>(seed),
                std::forward<Distribution&&>(distribution), multiplier);
        } else if constexpr (width == 1) {
            return generate<length, height, scaleY, scaleZ, overwrite>(generatedNoiseMap, std::forward<Seed&&>(seed),
                std::forward<Distribution&&>(distribution), multiplier);
        }
    ...
    template <int width, int length, int height, int scaleX, int scaleY, int scaleZ, int numOctaves = 8,
        typename Seed, typename Distribution, typename GeneratedNoiseType>
    constexpr void generateOctaves3DImpl(GeneratedNoiseType& generatedNoiseMap, Seed&& seed,
        ...
        if constexpr (numOctaves == 1) {
            ...
            generate<width, length, height, scaleX, scaleY, scaleZ, std::false_type>(generatedNoiseMap,
                std::forward<Seed&&>(seed), std::forward<Distribution&&>(distribution), multiplier);
            // ...then generate the next octaves.
            generateOctaves3DImpl<width, length, height, ceilDivide(scaleX, 2), ceilDivide(scaleY, 2),
                ceilDivide(scaleZ, 2), numOctaves - 1>(generatedNoiseMap, std::forward<Seed&&>(seed),
    ...
    template <int width, int length, int height, int scaleX, int scaleY, int scaleZ, int numOctaves = 8,
        typename overwrite = std::true_type, typename Seed = decltype(getCurrentTime()), typename Distribution
        = decltype(DefaultDistribution), typename GeneratedNoiseType>
    constexpr GeneratedNoiseType generateOctaves(GeneratedNoiseType& generatedNoiseMap, Seed&& seed = getCurrentTime(),
        Distribution&& distribution = std::forward<Distribution&&>(DefaultDistribution), float multiplier = 0.5f, float decayFactor = 0.5f) {
        if constexpr (overwrite::value) {
            generatedNoiseMap = 0.0f;
        }
        generateOctaves3DImpl<width, length, height, scaleX, scaleY, scaleZ, numOctaves>(generatedNoiseMap,
            std::forward<Seed&&>(seed), std::forward<Distribution&&>(distribution), multiplier, decayFactor);
        return generatedNoiseMap;
    }
