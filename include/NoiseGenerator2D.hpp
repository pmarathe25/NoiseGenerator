...
namespace StealthNoiseGenerator {
    ...
    template <int width, int length, int scaleX, int scaleY, typename overwrite = std::true_type,
        typename Seed = decltype(getCurrentTime()), typename Distribution = decltype(DefaultDistribution), typename GeneratedNoiseType>
    constexpr GeneratedNoiseType& generate(GeneratedNoiseType& generatedNoiseMap, Seed&& seed = getCurrentTime(),
        ...
        const auto& internalNoiseMap{generateInternalNoiseMap<internalWidth, internalLength>(std::forward<Seed&&>(seed),
    ...
    template <int width, int length, int scaleX, int scaleY, int numOctaves = 8, typename Seed, typename Distribution, typename GeneratedNoiseType>
    constexpr void generateOctaves2DImpl(GeneratedNoiseType& generatedNoiseMap, Seed&& seed, Distribution&& distribution, float multiplier, float decayFactor) {
